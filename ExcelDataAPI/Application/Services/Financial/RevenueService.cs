...
/// <summary>
/// Revenue Service Implementation - CHỈ XỬ LÝ REVENUE
/// Tách biệt hoàn toàn với Expense để dễ maintain và debug
/// </summary>
public class RevenueService : IRevenueService
{
    private readonly IDataProvider _dataProvider;

    public RevenueService(IDataProvider dataProvider)
    {
    ...
    public async Task<BulkOperationResultDto> BulkInsertAsync(List<RevenueImportDto> data)
    {
        ...
        try
        {
            ...
            // Step 3: Bulk insert
            var insertedRows = await _dataProvider.BulkInsertAsync(dataTable, DatabaseConstants.REVENUE_TABLE);
    ...
    public async Task<bool> CheckDataExistsAsync(int month, int year, int sourceId)
    {
        try
        {
            ...
            var sql = @"SELECT COUNT(1) FROM dbo.TaiChinh_ThuHoatDong 
    ...
    public async Task<List<RevenueImportDto>> GetByPeriodAsync(int month, int year)
    {
        try
        {
            // ✅ SECURE: Using parameterized query to prevent SQL injection
            var sql = @"SELECT * FROM dbo.TaiChinh_ThuHoatDong 
        ...
        {
            return new List<RevenueImportDto>();
        }
    }
    ...
    private static void ValidateRequiredFields(RevenueImportDto item, int rowNumber, ValidationResultDto result)
    {
        ...
        if (string.IsNullOrWhiteSpace(item.LoaiThu))
            result.AddError($"Dòng {rowNumber}: Loại thu không được trống");
    }

    private static void ValidateDataTypes(RevenueImportDto item, int rowNumber, ValidationResultDto result)
    {
        // Validate month
    ...
    private async Task CheckForDuplicates(RevenueImportDto item, int rowNumber, ValidationResultDto result)
    {
            ...
            int.TryParse(item.IdNguon, out int sourceId))
        {
            ...
            if (exists)
                result.AddWarning($"Dòng {rowNumber}: Revenue data có thể đã tồn tại (Tháng: {month}, Năm: {year}, Nguồn: {sourceId})");
        }
    }
    ...
    private static DataTable CreateDataTable(List<RevenueImportDto> data)
    {
        ...
        foreach (var item in data)
        {
            table.Rows.Add(
                ...
                item.LoaiThu ?? string.Empty,
                ParseDecimal(item.SoTien),
                item.MoTa ?? string.Empty,
                item.GhiChu ?? string.Empty,
                string.IsNullOrEmpty(item.ThoiGianNhap) ? DateTime.Now : ParseDateTime(item.ThoiGianNhap),
    ...
    private static List<RevenueImportDto> MapDataTableToDto(DataTable dataTable)
    {
        var result = new List<RevenueImportDto>();
        
        foreach (DataRow row in dataTable.Rows)
        {
            result.Add(new RevenueImportDto
            {
